...
- **verify 门禁 ✅**：`npm run verify` 全绿（lint / typecheck / test **371/371 + 1 环境跳过** / size / contract / spec **43/43** / e2e 浏览器矩阵）；**Phase 5 全部完成 + PLUG-012/013 完成**；下一步 = v1.0 收尾（遗留：插件运行时配置自动注册 / 云端 Space 托管（用户已排除）/ npm 包名与域名——见 PHASE-5-remaining-complete 交接）
