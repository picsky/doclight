...
**file:// 与 SSG 的边界（2026-08 补记）**：SSG 产物（dist-site）**必须通过 HTTP 服务器访问**
（`doclight preview` / 任意静态服务器）——SPA 导航依赖 fetch，file:// 下被浏览器 CORS 拦截、
绝对链接也会指向盘符根。SSG 页面内置 file:// 降级适配：双击打开时自动跳过展示层、站内链接
转为相对整页跳转（每页均为服务端直出的完整 HTML，可正常阅读与导航；TOC/搜索/主题等交互
降级为静态）。**离线双击的完整体验请用 `doclight bundle`**（见下）。
