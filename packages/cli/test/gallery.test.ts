...
describe("buildGallery（VIS-001 主题画廊；DP-001 单主题收敛）", () => {
  it("产物：索引 + 1 主题 × 亮/暗 共 3 个 HTML", () => {
    const dir = tmpDir();
    try {
  ...
  it("面板 fixedTheme 钉死模式（防闪烁脚本不读 localStorage）", () => {
    ...
    try {
      ...
      const dark = readFileSync(join(dir, "minimal", "dark", "index.html"), "utf8");
  ...
  it("示例文档覆盖全语法特性（标题/代码/表格/容器/公式/图表/引用/列表/Tabs/步骤）", () => {
    ...
    expect(SAMPLE_GALLERY_DOC).toContain(":::tabs");
    expect(SAMPLE_GALLERY_DOC).toContain(":::steps");
  ...
  it("面板内容与真实站点同构（同一渲染内核：示例文档渲染进 article）", () => {
    ...
    try {
      buildGallery({ outDir: dir });
      const panel = readFileSync(join(dir, "minimal", "light", "index.html"), "utf8");
      expect(panel).toContain("<article");
      expect(panel).toContain("中文排版优先"); // 示例文档渲染内容（首个 h1 由壳层承载）
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("build --themes 在产物中生成 gallery/（与站点同目录可部署）", () => {
    ...
    try {
      ...
      expect(existsSync(join(outDir, "gallery", "minimal", "dark", "index.html"))).toBe(true);
    } finally {
